...
static const arg_def_t selectopsarg =
    ARG_DEF(NULL, "select-op", 1,
            "Select OPS and OP (format: ops_id op_index, e.g., --select-op=1,2 "
            "(i.e., ops_id=1 op_index=2)");
#else
...
static const arg_def_t *all_args[] = {
  ...
  &skiparg,        &numstreamsarg, &summaryarg,
  &outputfile,
#if CONFIG_PARAKIT_COLLECT_DATA
  &datafilesuffix, &datafilepath,
#endif
  &threadsarg,     &verbosearg,    &scalearg,
  &fb_arg,         &md5arg,        &verifyarg,
  &framestatsarg,  &continuearg,   &outbitdeptharg,
#if CONFIG_CWG_F429_INTEROP
  &selectopsarg,
#else
  &oppointarg,
#endif  //  CONFIG_CWG_F429_INTEROP
  &outallarg,      &skipfilmgrain, &randomaccess,
