...
static void open_multiple_output_files_perview(
    ...
    FILE **outfile) {
  const int num_views = aom_input_ctx.num_views;
  for (int view_id = 0; view_id < num_views; view_id++) {
    ...
    // add suffix only for multiple views (backward compatible when num_views ==
    // 1)
    strcat(output_filename, num_views > 1 ? suffix : "");
