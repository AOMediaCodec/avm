  ...
  if (ti->uniform_spacing) {
    ...
    *max_w = AVMMAX(tile_width_sb, last_tile_width_sb);
    ...
    for (int i = 0; i < ti->cols; i++) {
      ...
      size_sb = size_sb * sb_cols / seq_sb_cols;
  ...
  if (ti->uniform_spacing) {
    ...
    *max_h = AVMMAX(tile_height_sb, last_tile_height_sb);
    ...
    for (int i = 0; i < ti->rows; i++) {
      ...
      size_sb = size_sb * sb_rows / seq_sb_rows;
      if (size_sb <= 0) return 0;
      min_val = AVMMIN(min_val, size_sb);
      max_val = AVMMAX(max_val, size_sb);
    }
...
int av2_check_valid_tile_set(const SequenceHeader *seq_params,
                             const TileInfoSyntax *tile_params, int sb_cols,
                             int sb_rows) {
  int max_tile_width_limit;
  int max_tile_area_limit;

  if (seq_params->seq_max_level_idx == SEQ_LEVEL_MAX) {
    max_tile_width_limit = INT_MAX;
    max_tile_area_limit = INT_MAX;
  } else {
    const int tier = (seq_params->seq_tier > 0) ? 1 : 0;
    const int level = seq_params->seq_max_level_idx;
    const int sb_size_log2 =
        tile_params->tile_info.mib_size_log2 + MI_SIZE_LOG2;
  ...
  int min_w, max_w;
  if (!av2_compute_tile_width_range(tile_params, sb_cols, &min_w, &max_w))
    return 0;
  if (min_w < 1) return 0;
  if (max_w > max_tile_width_limit) return 0;

  int min_h, max_h;
  if (!av2_compute_tile_height_range(tile_params, sb_rows, &min_h, &max_h))
    return 0;
  if (min_h < 1) return 0;
  if ((int64_t)max_w * max_h > max_tile_area_limit) return 0;
