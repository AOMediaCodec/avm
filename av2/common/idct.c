...
void inv_txfm_c(const tran_low_t *input, uint16_t *dest, int stride,
  ...
  if (txfm_param->lossless) {
    assert(primary_tx_type == DCT_DCT);
    av2_highbd_iwht4x4_add(input, dest, stride, txfm_param->eob,
                           txfm_param->bd);
    return;
  }

  int tx_type_row = g_hor_tx_type[primary_tx_type];
  int tx_type_col = g_ver_tx_type[primary_tx_type];
...
void av2_highbd_inv_txfm_add_4x4_vert_c(const tran_low_t *input, uint16_t *dest,
                                        int stride,
                                        const TxfmParam *txfm_param) {
  assert(av2_ext_tx_used[txfm_param->tx_set_type][txfm_param->primary_tx_type]);
  int eob = txfm_param->eob;
  int bd = txfm_param->bd;
  int lossless = txfm_param->lossless;
  const PRIMARY_TX_TYPE primary_tx_type = txfm_param->primary_tx_type;
  if (lossless) {
    assert(primary_tx_type == DCT_DCT);
    (void)primary_tx_type;
...
static void init_txfm_param(const MACROBLOCKD *xd, int plane, TX_SIZE tx_size,
  ...
  bool mode_dependent_condition =
      (txfm_param->is_inter ? (txfm_param->primary_tx_type == DCT_DCT &&
                               width >= 16 && height >= 16)
...
void av2_inverse_transform_block(const MACROBLOCKD *xd,
  ...
  assert(IMPLIES(txfm_param.sec_tx_type,
                 block_signals_sec_tx_type(xd, tx_size, tx_type.primary_tx,
                                           txfm_param.eob)));
...
void av2_inv_stxfm(tran_low_t *coeff, TxfmParam *txfm_param) {
  ...
  if ((width >= 4 && height >= 4) && stx_type) {
    ...
    fprintf(stderr,
            ...
            "(stx coeff)\n",
            txfm_param->is_inter, txfm_param->primary_tx_type, width, height,
