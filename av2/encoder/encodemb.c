...
static AVM_INLINE void fill_residue_outside_frame(
    ...
    int visible_tx_cols, int visible_tx_rows, PRIMARY_TX_TYPE primary_tx_type) {
  const int complete_block_outside =
      (visible_tx_cols == 0 || visible_tx_rows == 0);

  if (primary_tx_type <= IDTX) {
    int16_t avg = 0;
    if (primary_tx_type != IDTX && !complete_block_outside)
      avg =
  ...
  } else if (htx_tab[primary_tx_type] == IDTX_1D) {
    ...
    assert(vtx_tab[primary_tx_type] == IDTX_1D);
...
void av2_subtract_block(const MACROBLOCKD *xd, int rows, int cols,
                        ...
                        int frame_height, PRIMARY_TX_TYPE primary_tx_type) {
  ...
  if (is_border_block) {
    fill_residue_outside_frame(diff, diff_stride, cols, rows, visible_tx_cols,
                               visible_tx_rows, primary_tx_type);
  }
}
...
int av2_optimize_b(const struct AV2_COMP *cpi, MACROBLOCK *x, int plane,
  ...
  const TX_CLASS tx_class = tx_type_to_class[tx_type.primary_tx];
...
void parity_hiding_trellis_off(const struct AV2_COMP *cpi, MACROBLOCK *mb,
  ...
  const qm_val_t *iqmatrix = av2_get_iqmatrix(
      &cpi->common.quant_params, xd, plane_type, tx_size, tx_type.primary_tx);
  ...
  const SCAN_ORDER *const scan_order = get_scan(tx_size, tx_type.primary_tx);
...
void av2_xform_quant(const int use_tcq_deadzone_boost, const AV2_COMMON *cm,
  ...
  const uint8_t fsc_mode =
      ((cm->seq_params.enable_fsc &&
        ...
        plane == PLANE_TYPE_Y) ||
       use_inter_fsc(cm, plane, txfm_param->primary_tx_type, is_inter));
  av2_quant(use_tcq_deadzone_boost, x, plane, block, txfm_param, qparam);
  if (fsc_mode) {
    if (txfm_param->primary_tx_type == IDTX) {
...
void set_bob(MACROBLOCK *x, int plane, int block, TX_SIZE tx_size,
             PRIMARY_TX_TYPE primary_tx_type) {
  const struct macroblock_plane *const p = &x->plane[plane];
  const SCAN_ORDER *const scan_order = get_scan(tx_size, primary_tx_type);
  const int block_offset = BLOCK_OFFSET(block);
...
void av2_quant(const int use_tcq_deadzone_boost, MACROBLOCK *x, int plane,
  ...
  set_bob(x, plane, block, txfm_param->tx_size, txfm_param->primary_tx_type);
...
void av2_setup_xform(const AV2_COMMON *cm, MACROBLOCK *x, int plane,
  ...
  txfm_param->primary_tx_type = tx_type.primary_tx;
  ...
  bool mode_dependent_condition =
      (txfm_param->is_inter
           ? (txfm_param->primary_tx_type == DCT_DCT && width >= 16 &&
  ...
  if (mode_dependent_condition && !xd->lossless[mbmi->segment_id] &&
    ...
    if (!is_inter_block(xd->mi[0], xd->tree_type)) {
      ...
      if (txfm_param->primary_tx_type == ADST_ADST) {
...
void av2_setup_qmatrix(const CommonQuantParams *quant_params,
                       ...
                       PRIMARY_TX_TYPE primary_tx_type, QUANT_PARAM *qparam) {
  qparam->qmatrix =
      av2_get_qmatrix(quant_params, xd, plane, tx_size, primary_tx_type);
  qparam->iqmatrix =
      av2_get_iqmatrix(quant_params, xd, plane, tx_size, primary_tx_type);
}
...
static void encode_block(int plane, int block, int blk_row, int blk_col,
  ...
  for (int i = plane; i <= plane_end; i++) {
    ...
    av2_subtract_txb(x, i, plane_block_size, blk_col, blk_row, tx_size,
                     cm->width, cm->height, plane_tx_type.primary_tx);
  }
  CctxType cctx_type =
  ...
  if (!is_blk_skip(x->txfm_search_info.blk_skip[plane],
    ...
    av2_setup_qmatrix(&cm->quant_params, xd, plane, tx_size, tx_type.primary_tx,
                      &quant_param);
    ...
    bool enable_parity_hiding =
        ...
        plane == PLANE_TYPE_Y && ph_allowed_tx_types[tx_type.primary_tx] &&
    ...
    const PRIMARY_TX_TYPE primary_tx_type = tx_type.primary_tx;
    const SEC_TX_TYPE stx = tx_type.sec_tx;
    if (p->eobs[block] == 1 && plane == PLANE_TYPE_Y && !is_inter) {
      if (tx_type.primary_tx != DCT_DCT || stx > 0) {
        tx_type = MAKE_TX_TYPE_FROM_PRIMARY_TX_TYPE(DCT_DCT);
        update_txk_array(xd, blk_row, blk_col, tx_size, tx_type);
      }
    }
    if (p->eobs[block] <= 3 && plane == PLANE_TYPE_Y && is_inter && stx) {
      tx_type = MAKE_TX_TYPE_FROM_PRIMARY_TX_TYPE(primary_tx_type);
      update_txk_array(xd, blk_row, blk_col, tx_size, tx_type);
    }
...
void av2_encode_block_intra(int plane, int block, int blk_row, int blk_col,
  ...
  TX_TYPE tx_type = MAKE_TX_TYPE_FROM_PRIMARY_TX_TYPE(DCT_DCT);
  const int bw = mi_size_wide[plane_bsize];
  ...
  if (plane == 0 && is_blk_skip(x->txfm_search_info.blk_skip[plane],
    ...
    *bob_code = 0;
    ...
    tx_type = av2_get_tx_type(xd, plane_type, blk_row, blk_col, tx_size,
                              is_reduced_tx_set_used(cm, plane_type));
    PRIMARY_TX_TYPE primary_tx_type =
        is_stat_generation_stage(cpi) ? DCT_DCT : tx_type.primary_tx;
    ...
    if (*eob == 1 && (tx_type.primary_tx != DCT_DCT || tx_type.sec_tx > 0) &&
        plane == 0) {
