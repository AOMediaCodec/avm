...
void av2_write_coeffs_txb(const AV2_COMMON *const cm, MACROBLOCK *const x,
  ...
  bool enable_parity_hiding =
      ...
      ph_allowed_tx_types[tx_type.primary_tx] && (eob > PHTHRESH);
              ...
              for (int txb_idx = 0; txb_idx < txb_pos.n_partitions; ++txb_idx) {
                ...
                if (code_rest) {
                  if (((cm->seq_params.enable_fsc &&
                        ...
                        tx_type.primary_tx == IDTX && plane == PLANE_TYPE_Y) ||
                       use_inter_fsc(cm, plane, tx_type.primary_tx,
                                     is_inter))) {
...
int get_tx_type_cost(const MACROBLOCK *x, const MACROBLOCKD *xd, int plane,
  ...
  if (xd->lossless[xd->mi[0]->segment_id]) {
    if (is_inter && tx_size == TX_4X4) {
      int lossless_inter_tx_type = tx_type.primary_tx == IDTX;
    ...
    if (is_inter) {
      if (ext_tx_set > 0) {
        ...
        if (tx_set_type != EXT_TX_SET_LONG_SIDE_64 &&
          ...
          int tx_type_idx = av2_ext_tx_ind[tx_set_type][tx_type.primary_tx];
          ...
          bool is_long_side_dct = is_dct_type(tx_size, tx_type.primary_tx);
          ...
          int tx_idx_for_large_txfm = get_idx_from_txtype_for_large_txfm(
              tx_set_type, tx_type.primary_tx,
        ...
        if (block_signals_sec_tx_type(xd, tx_size, tx_type.primary_tx, eob) &&
            xd->enable_ist) {
          tx_type_cost +=
              x->mode_costs
      ...
      if (ext_tx_set > 0) {
        ...
        if (eob != 1) {
          ...
          const int tx_type_idx = av2_tx_type_to_idx(
              tx_type.primary_tx, tx_set_type, intra_dir, size_info);
          if (tx_set_type != EXT_TX_SET_LONG_SIDE_64 &&
              tx_set_type != EXT_TX_SET_LONG_SIDE_32) {
            tx_type_cost +=
...
int av2_cost_coeffs_txb_estimate(const MACROBLOCK *x, const int plane,
  ...
  const SCAN_ORDER *scan_order = get_scan(tx_size, tx_type.primary_tx);
...
int av2_optimize_fsc_block(const struct AV2_COMP *cpi, MACROBLOCK *x, int plane,
  ...
  const qm_val_t *iqmatrix = av2_get_iqmatrix(
      &cpi->common.quant_params, xd, plane, tx_size, tx_type.primary_tx);
  ...
  const int is_fsc =
      ((cm->seq_params.enable_fsc &&
        xd->mi[0]->fsc_mode[xd->tree_type == CHROMA_PART] &&
        plane == PLANE_TYPE_Y) ||
       use_inter_fsc(&cpi->common, plane, tx_type.primary_tx, is_inter));
  const LV_MAP_COEFF_COST *txb_costs =
...
int av2_optimize_txb_new(const struct AV2_COMP *cpi, MACROBLOCK *x, int plane,
  ...
  set_bob(x, plane, block, tx_size, tx_type.primary_tx);
...
static void update_tx_type_count(const AV2_COMP *cpi, const AV2_COMMON *cm,
  ...
  if (is_inter) {
    if (cpi->oxcf.txfm_cfg.use_inter_dct_only) {
      assert(tx_type.primary_tx == DCT_DCT && tx_type.sec_tx == 0);
    }
  } else {
    if (cpi->oxcf.txfm_cfg.use_intra_dct_only) {
      assert(tx_type.primary_tx == DCT_DCT ||
             (cm->seq_params.enable_idtx_intra && tx_type.primary_tx == IDTX));
    } else if (cpi->oxcf.txfm_cfg.use_intra_default_tx_only) {
      const PRIMARY_TX_TYPE default_type = get_default_tx_type(
          PLANE_TYPE_Y, xd, tx_size, cpi->is_screen_content_type);
      (void)default_type;
      // assert(tx_type.primary_tx == default_type);
    }
  }
    ...
    if (eset > 0) {
      ...
      if (is_inter) {
        ...
        if (allow_update_cdf && cm->seq_params.enable_inter_ist &&
            block_signals_sec_tx_type(xd, tx_size, tx_type.primary_tx, eob)) {
          ...
          if (allow_update_cdf) {
            update_cdf(
                fc->intra_ext_tx_cdf[eset +
                                    ...
                                    [txsize_sqr_map[tx_size]],
                av2_tx_type_to_idx(tx_type.primary_tx, tx_set_type, intra_dir,
                                   av2_size_class[tx_size]),
                cm->features.reduced_tx_set_used
...
void av2_update_and_record_txb_skip_context(int plane, int block, int blk_row,
  ...
  if (args->dry_run == OUTPUT_ENABLED) {
    ...
    td->rd_counts.tx_type_used[tx_size][tx_type.primary_tx]++;
    int bob = av2_get_max_eob(tx_size) - bob_code;
#if CONFIG_ENTROPY_STATS
