...
static int av2_pack_bitstream_internal(AV2_COMP *const cpi, uint8_t *dst,
  ...
  if (cm->current_frame.cm_obu_type == OBU_CLK) {
    ...
    if (!cpi->oxcf.unit_test_cfg.single_seq_header_for_all_test ||
        cpi->write_sequence_header_flag) {
      obu_header_size =
      ...
      data += obu_header_size + obu_payload_size + length_field_size;
      if (cpi->oxcf.unit_test_cfg.single_seq_header_for_all_test)
        cpi->write_sequence_header_flag = 0;
    }
  ...
  // write an extra sequence header mid-GOP for test purposes
  if (cpi->oxcf.unit_test_cfg.add_seq_header_in_GOP_test &&
      cm->current_frame.frame_number ==
          cpi->oxcf.unit_test_cfg.add_seq_header_in_GOP_test) {
    size_t length_field_size = 0;
    obu_header_size =
        av2_write_obu_header(level_params, OBU_SEQUENCE_HEADER, 0, 0, data);
    if (cm->seq_params.seq_seg_info_present_flag)
