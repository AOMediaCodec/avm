...
static void write_ops_mlayer_info(struct OpMLayerInfo *ops_layer_map, int xLId,
...
static uint32_t calculate_ops_data_size(AV2_COMP *cpi, int obu_xlayer_id,
  ...
  if (ops->ops_intent_present_flag)
    avm_wb_write_literal(&temp_wb, ops->ops_intent, 7);

  // Write PTL fiels if present
  if (ops->ops_ptl_present_flag) {
    if (obu_xlayer_id == GLOBAL_XLAYER_ID) {
      avm_wb_write_literal(&temp_wb, op->ops_config_idc, CONFIG_BITS);
  ...
  if (ops->ops_color_info_present_flag)
    write_ops_color_info(&op->col_info, &temp_wb);

  if (ops->ops_decoder_model_info_present_flag)
    write_ops_decoder_model_info(&op->decoder_model_info, &temp_wb);

  avm_wb_write_bit(&temp_wb, op->ops_initial_display_delay_present_flag);
  if (op->ops_initial_display_delay_present_flag)
    avm_wb_write_literal(&temp_wb, op->ops_initial_display_delay_minus_1, 4);
      ...
      if (op->ops_xlayer_map & (1 << j)) {
        ...
        if (ops->ops_mlayer_info_idc == 1) {
          ...
          if (op->ops_mlayer_explicit_info_flag[j]) {
            ...
            avm_wb_write_literal(&temp_wb, op->ops_embedded_mapping[j], 4);
            avm_wb_write_literal(&temp_wb, op->ops_embedded_op_id[j], 3);
          }
        }
      }
    }
...
uint32_t av2_write_operating_point_set_obu(AV2_COMP *cpi, int ops_id,
...
#if CONFIG_AV2_PROFILES
  // Have to use ops_id
  struct OperatingPointSet *ops = &cpi->ops_list[ops_id];
  int obu_xlayer_id = ops->obu_xlayer_id;
  ...
  for (int i = 0; i < ops->ops_cnt; i++) {
    ...
    if (ops->ops_intent_present_flag)
      avm_wb_write_literal(&wb, op->ops_intent_op, 4);

    if (ops->ops_ptl_present_flag) {
      if (obu_xlayer_id == GLOBAL_XLAYER_ID) {
        avm_wb_write_literal(&wb, op->ops_config_idc, CONFIG_BITS);
        ...
        avm_wb_write_literal(&wb, op->ops_seq_profile_idc[i], PROFILE_BITS);
        avm_wb_write_literal(&wb, op->ops_level_idx[i], LEVEL_BITS);
        avm_wb_write_bit(&wb, op->ops_tier_flag[i]);
        avm_wb_write_literal(&wb, op->ops_mlayer_count[i], 3);
      }
    }
    if (ops->ops_color_info_present_flag)
      write_ops_color_info(&op->col_info, &wb);

    if (ops->ops_decoder_model_info_present_flag) {
      write_ops_decoder_model_info(&op->decoder_model_info, &wb);
    }

    avm_wb_write_bit(&wb, op->ops_initial_display_delay_present_flag);
    if (op->ops_initial_display_delay_present_flag) {
      avm_wb_write_literal(&wb, op->ops_initial_display_delay_minus_1, 4);
    }
        ...
        if ((op->ops_xlayer_map & (1 << j))) {
          ...
          if (ops->ops_mlayer_info_idc == 1) {
            ...
            if (op->ops_mlayer_explicit_info_flag[j]) {
              ...
              avm_wb_write_literal(&wb, op->ops_embedded_mapping[j], 4);
              avm_wb_write_literal(&wb, op->ops_embedded_op_id[j], 3);
            }
          }
        }
      }
...
void av2_set_ops_params(struct OperatingPointSet *ops, int xlayer_id,
  ...
  ops->ops_id = ops_id;
  ops->ops_cnt = ops_cnt;
  // Set default mlayer_info_udc based on xlayer_id
