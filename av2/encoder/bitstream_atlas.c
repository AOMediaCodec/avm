...
// Updated functions for simplified structures without multi-dimensional arrays

static uint32_t write_ats_region_info(struct AtlasRegionInfo *atlas_reg_params,
                                      struct avm_write_bit_buffer *wb) {
  avm_wb_write_uvlc(wb, atlas_reg_params->ats_num_region_columns_minus1);
  avm_wb_write_uvlc(wb, atlas_reg_params->ats_num_region_rows_minus1);

  avm_wb_write_bit(wb, atlas_reg_params->ats_uniform_spacing_flag);
  if (!atlas_reg_params->ats_uniform_spacing_flag) {
    for (int i = 0; i < atlas_reg_params->ats_num_region_columns_minus1 + 1;
         i++) {
      avm_wb_write_uvlc(wb, atlas_reg_params->ats_column_width_minus1[i]);
    }
    for (int i = 0; i < atlas_reg_params->ats_num_region_rows_minus1 + 1; i++) {
      avm_wb_write_uvlc(wb, atlas_reg_params->ats_row_height_minus1[i]);
    }
  } else {
...
static uint32_t write_ats_multistream_atlas_info(
    ...
    struct avm_write_bit_buffer *wb) {
  avm_wb_write_uvlc(wb, ats_multistream_info->ats_atlas_width);
  avm_wb_write_uvlc(wb, ats_multistream_info->ats_atlas_height);
  avm_wb_write_uvlc(wb, ats_multistream_info->ats_num_atlas_segments_minus1);

  // Handle hasAlpha parameter as per updated spec
  if (hasAlpha) {
    avm_wb_write_bit(wb, ats_multistream_info->ats_alpha_segments_present_flag);
  }

  avm_wb_write_bit(wb, ats_multistream_info->ats_background_info_present_flag);
  if (ats_multistream_info->ats_background_info_present_flag == 1) {
    avm_wb_write_literal(wb, ats_multistream_info->ats_background_red_value, 8);
    avm_wb_write_literal(wb, ats_multistream_info->ats_background_green_value,
                         8);
    avm_wb_write_literal(wb, ats_multistream_info->ats_background_blue_value,
                         8);
  }

  const int NumSegments =
      ats_multistream_info->ats_num_atlas_segments_minus1 + 1;
  for (int i = 0; i < NumSegments; i++) {
    avm_wb_write_literal(wb, ats_multistream_info->ats_input_stream_id[i], 5);
    avm_wb_write_uvlc(wb, ats_multistream_info->ats_segment_top_left_pos_x[i]);
    avm_wb_write_uvlc(wb, ats_multistream_info->ats_segment_top_left_pos_y[i]);
    avm_wb_write_uvlc(wb, ats_multistream_info->ats_segment_width[i]);
    avm_wb_write_uvlc(wb, ats_multistream_info->ats_segment_height[i]);
    ...
    if (ats_multistream_info->ats_alpha_segments_present_flag == 1 &&
      ...
      avm_wb_write_bit(wb, ats_multistream_info->ats_alpha_segment_flag[i]);
    }
  }
  return 0;
}
...
static uint32_t write_ats_basic_atlas_info(
    struct AtlasBasicAtlasInfo *ats_basic_info,
    struct avm_write_bit_buffer *wb) {
  ...
  avm_wb_write_uvlc(wb, ats_basic_info->ats_num_atlas_segments_minus1);

  const int NumSegments = ats_basic_info->ats_num_atlas_segments_minus1 + 1;
  for (int i = 0; i < NumSegments; i++) {
...
static uint32_t write_ats_region_to_segment_mapping(
  ...
  if (!ats_reg_seg_map->ats_single_region_per_atlas_segment_flag) {
    avm_wb_write_uvlc(wb, ats_reg_seg_map->ats_num_atlas_segments_minus1);
    const int NumSegments = ats_reg_seg_map->ats_num_atlas_segments_minus1 + 1;
    for (int i = 0; i < NumSegments; i++) {
...
static uint32_t write_ats_label_segment_info(
    struct AtlasLabelSegmentInfo *ats_label, int NumSegments,
    struct avm_write_bit_buffer *wb) {
  assert(NumSegments >= 0);

  avm_wb_write_bit(wb, ats_label->ats_signaled_atlas_segment_ids_flag);
...
uint32_t av2_write_atlas_segment_info_obu(AV2_COMP *cpi, int xLayer_id,
                                          int atlas_seg_id,
                                          uint8_t *const dst) {
  ...
  struct AtlasSegmentInfo *atlas_params =
      &cpi->atlas_list[xLayer_id][atlas_seg_id];
  ...
  if (atlas_params->atlas_segment_mode_idc == ENHANCED_ATLAS) {
    write_ats_region_info(&atlas_params->ats_region_info, &wb);
    write_ats_region_to_segment_mapping(
        &atlas_params->ats_region_to_segment_mapping,
        atlas_params->ats_region_info.NumRegionsInAtlas, &wb);
    ...
    write_ats_basic_atlas_info(&atlas_params->ats_basic_atlas_info, &wb);
    num_segments =
    ...
    write_ats_multistream_atlas_info(&atlas_params->ats_multistream_atlas_info,
                                     0, &wb);
    num_segments =
  ...
  write_ats_label_segment_info(&atlas_params->ats_label_segment_info,
                               num_segments, &wb);
