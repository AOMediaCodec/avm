...
static AVM_INLINE int handle_warp_delta_mode(
  ...
  if (!allow_warp_parameter_signaling(cm, mbmi)) {
    ...
    if (!cpi->sf.part_sf.enable_six_param_warp_in_winner_mode ||
        !eval_motion_mode)
    ...
    const int six_param_enabled_by_tid =
        ...
        cpi->sf.part_sf.enable_six_param_warp_in_winner_mode_by_tid;
    const bool use_six_param_in_winner =
        (eval_motion_mode == six_param_enabled_by_tid);
    if (use_six_param_in_winner ||
        !cpi->sf.part_sf.enable_six_param_warp_in_winner_mode)
...
static AVM_INLINE int is_check_in_winner(AV2_COMP *cpi,
                                         MB_MODE_INFO *const mbmi) {
  if (!cpi->sf.part_sf.enable_six_param_warp_in_winner_mode) return 0;
