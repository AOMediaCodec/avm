...
// Returns 1 when the current block configuration is the "anchor": the baseline
// value across every active inter-prediction dimension. The anchor is always
// given a full RD evaluation (never pruned by the model-RD heuristic) so that
// the mode search keeps a reliable reference point.
static int motion_mode_is_prune_exempt(const AV2_COMMON *const cm,
                                       const MACROBLOCKD *const xd,
                                       BLOCK_SIZE bsize) {
  const MB_MODE_INFO *const mbmi = xd->mi[0];

  // Motion mode: only SIMPLE_TRANSLATION, or a warp mode whose warp side-info
  // is all at baseline. INTERINTRA is never the anchor.
...
static int prune_motion_mode(int64_t this_model_rd,
  ...
  // Never prune the anchor configuration, even when its model RD does not rank.
...
void av2_rd_pick_inter_mode_sb(struct AV2_COMP *cpi,
  ...
  // The static evaluation LUT places TIP entries first, so TIP would win on
  // base-layer blocks before any real-ref candidate populates best_rd. In
  // base-only implicit-ref-map decode the TIP virtual reference is derived
  // from frames that may be absent, so a TIP-winning base-layer block would
  // not be correctly decodable. Suppress TIP entirely on such frames.
  const int base_only_implicit_risk =
      cm->number_mlayers > 1 && cm->mlayer_id == 0 &&
      !cm->seq_params.enable_explicit_ref_frame_map;
  const int tip_allowed = is_tip_allowed(cm, xd) && !base_only_implicit_risk;
  ...
  for (int eval_idx = 0; eval_idx < ref_frame_centric_eval_order_num;
       ++eval_idx) {
    if (bsize == BLOCK_4X4) break;
    const PREDICTION_MODE this_mode =
        ref_frame_centric_eval_order[eval_idx].mode;
    const MV_REFERENCE_FRAME ref_frame =
        ref_frame_centric_eval_order[eval_idx].rf0;
    const MV_REFERENCE_FRAME second_ref_frame =
        ref_frame_centric_eval_order[eval_idx].rf1;
