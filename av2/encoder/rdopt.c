...
static AVM_INLINE void update_inter_mode_rd(HandleInterModeArgs *args,
  ...
  if (use_amvd < 0 || use_amvd >= NUM_USE_AMVD_OPTIONS) {
    assert(0 &&
           "use_amvd out of range; upstream amvd loop should bound to 0/1");
    return;
  }
  const int rf0 = COMPACT_INDEX0_NRS(mbmi->ref_frame[0]);
  if (rf0 < 0 || rf0 >= SINGLE_REF_FRAMES) {
    assert(0 &&
           "rf0 out of range; is_inter_singleref_mode should ensure valid ref");
    return;
  }
...
// should be skipped because the matching anchor RD is not within
// (1 / slack_denom) of the best anchor RD across all ref frames. Decision
// rule: keep when EITHER full_rd OR fast_rd is within slack of its respective
// best; prune when neither is (this includes the no-anchor-data case).
// slack_denom = 10 means 10% slack; slack_denom = 4 means 25% slack; lower
// values are looser.
static AVM_INLINE int prune_single_by_anchor_rd(
    const InterModeRdPair anchor_rd[SINGLE_REF_FRAMES], int this_ref_idx,
    int slack_denom) {
  if (this_ref_idx < 0 || this_ref_idx >= SINGLE_REF_FRAMES) return 0;
  if (slack_denom <= 0) return 0;
  if (anchor_rd[this_ref_idx].full_rd != INT64_MAX) {
    int64_t best_full = INT64_MAX;
    for (int r = 0; r < SINGLE_REF_FRAMES; ++r) {
      if (anchor_rd[r].full_rd < best_full) best_full = anchor_rd[r].full_rd;
    }
    if (anchor_rd[this_ref_idx].full_rd <=
        best_full + best_full / slack_denom) {
      return 0;
    }
  }
  if (anchor_rd[this_ref_idx].fast_rd != INT64_MAX) {
    int64_t best_fast = INT64_MAX;
    for (int r = 0; r < SINGLE_REF_FRAMES; ++r) {
      if (anchor_rd[r].fast_rd < best_fast) best_fast = anchor_rd[r].fast_rd;
    }
    if (anchor_rd[this_ref_idx].fast_rd <=
...
static AVM_INLINE int prune_newmv_modes_by_prior_rd(
  ...
  if (this_mode == NEWMV) {
    ...
    return prune_single_by_anchor_rd(prior, rf_idx, slack_denom);
  }
