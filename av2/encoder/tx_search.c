  ...
  if (!is_inter && best_eob &&
    ...
    if (do_quant) {
      ...
      av2_setup_qmatrix(&cm->quant_params, xd, plane, tx_size,
                        get_primary_tx_type(best_tx_type), &quant_param_intra);
      ...
      const uint8_t fsc_mode =
          ((cm->seq_params.enable_fsc &&
            ...
            plane == PLANE_TYPE_Y) ||
           use_inter_fsc(cm, plane, get_primary_tx_type(best_tx_type),
                         0 /*is_inter*/));
...
get_tx_mask(const AV2_COMP *cpi, MACROBLOCK *x, int plane, int block,
  ...
  if (plane) {
    ...
    uv_primary_tx_type = txk_allowed = get_primary_tx_type(uv_tx_type);
  }
    ...
    for (int set_idx = 0; set_idx < max_set_id; ++set_idx) {
      ...
      for (int stx = init_stx; stx < max_stx; ++stx) {
        ...
        *coeffs_available = 1;
        ...
        if (use_tcq) {
          skip_trellis_based_on_satd[txfm_param.primary_tx_type] = skip_trellis;
        } else {
          skip_trellis_based_on_satd[txfm_param.primary_tx_type] =
