...
void av2_update_nbr_diagonal_neon(struct tcq_ctx_t *tcq_ctx, int row, int col,
  ...
  uint8x8_t l0m1 = vmin_u8(prev_lev1, max1_v);
  uint8x8_t l1m1 = vmin_u8(prev_lev2, max1_v);
  uint8x8_t l0m2 = vmin_u8(prev_lev1, max_v);
  uint8x8_t l1m2 = vmin_u8(prev_lev2, max_v);

  for (int i = idx0; i < idx_end; i++) {
    uint8x8_t old_base = vtbl1_u8(vld1_u8(tcq_ctx->mag_base[i]), orig_v);
    uint8x8_t old_mid = vtbl1_u8(vld1_u8(tcq_ctx->mag_mid[i]), orig_v);
