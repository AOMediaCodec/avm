...
static bool is_compressed_size_obu(int obu_type) {
  switch (obu_type) {
    ...
    case OBU_RAS_FRAME: return true;
...
static bool compressed_size_for_context(Av2DecoderModelVerifier *verifier,
  ...
  for (size_t i = 0; i < verifier->current_tu_obu_count; ++i) {
    ...
    if (!is_compressed_size_obu(obu->obu_type) ||
...
void av2_decoder_model_verifier_record_obu(AV2Decoder *pbi, int obu_type,
  ...
  event->raw_obu.obu_type = obu_type;
  event->raw_obu.xlayer_id = xlayer_id;
  event->raw_obu.mlayer_id = mlayer_id;
  event->raw_obu.temporal_id = temporal_id;
  event->raw_obu.bits = obu_bits;
  event->raw_obu.frame_unit_index = verifier->source_frame_unit_index;
  event->raw_obu.temporal_unit_index = verifier->temporal_unit_index;
  event->raw_obu.event_index = event->index;
  event->raw_obu.decoder_retained = true;
  ...
  verifier->current_tu_obus[verifier->current_tu_obu_count] = event->raw_obu;
  ...
  for (size_t i = 0; i < verifier->context_count; ++i) {
    Av2DmContext *const context = &verifier->contexts[i];
    if (context->active && obu_belongs_to_context(context, &event->raw_obu) &&
...
void av2_decoder_model_verifier_on_obu_filtered(AV2Decoder *pbi) {
  ...
  if (event->type == AV2_DM_ADAPTER_RAW_OBU) {
    event->decoder_retained = false;
    event->raw_obu.decoder_retained = false;
    for (size_t i = verifier->current_tu_obu_count; i > 0; --i) {
      Av2DmPendingObu *const obu = &verifier->current_tu_obus[i - 1];
      if (obu->event_index == event->raw_obu.event_index) {
...
void av2_decoder_model_verifier_on_frame_unit_complete(AV2Decoder *pbi) {
  ...
  event->raw_obu.xlayer_id = snapshot.xlayer_id;
  event->raw_obu.mlayer_id = snapshot.mlayer_id;
  event->raw_obu.temporal_id = snapshot.temporal_id;
  if (!snapshot.show_existing_frame) {
    for (size_t i = 0; i < verifier->context_count; ++i) {
      Av2DmContext *const context = &verifier->contexts[i];
      if (!context->active ||
...
void av2_decoder_model_verifier_on_olk_reference_invalidation(
    AV2Decoder *pbi, uint32_t ref_valid_mask) {
  if (pbi == NULL || pbi->decoder_model_verifier == NULL) return;
  Av2DecoderModelVerifier *const verifier = pbi->decoder_model_verifier;
  if (!verifier_accepts_events(verifier)) return;
  Av2DmAdapterEvent *const adapter_event =
...
void av2_decoder_model_verifier_after_reference_update(
    AV2Decoder *pbi, uint32_t refresh_frame_flags, uint32_t ref_valid_mask) {
  if (pbi == NULL || pbi->decoder_model_verifier == NULL) return;
  Av2DecoderModelVerifier *const verifier = pbi->decoder_model_verifier;
  if (!verifier_accepts_events(verifier)) return;
  Av2DmAdapterEvent *const adapter_event =
