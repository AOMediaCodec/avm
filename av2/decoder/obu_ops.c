...
static void read_ops_mlayer_info(
    ...
    int xLId, struct OpMLayerInfo *ops_mlayer_info,
    struct avm_read_bit_buffer *rb) {
#if CONFIG_AV2_PROFILES
  // mlayer map
  ops_mlayer_info->ops_mlayer_map[xLId] =
      avm_rb_read_literal(rb, MAX_NUM_MLAYERS);
  ops_mlayer_info->OPMLayerCount[xLId] = 0;
  int mCount = 0;
...
uint32_t av2_read_operating_point_set_obu(struct AV2Decoder *pbi,
  ...
  // exisiting OPS
  struct OperatingPointSet *ops = &pbi->ops_list[ops_id];
  ops->valid = 1;
  ...
  if (ops->ops_cnt > 0) {
    ...
    for (int i = 0; i < ops->ops_cnt; i++) {
      ...
      op->ops_data_size = signaled_ops_data_size;
      ...
      if (ops->ops_ptl_present_flag) {
        if (obu_xlayer_id == GLOBAL_XLAYER_ID) {
          op->ops_config_idc = avm_rb_read_literal(rb, CONFIG_BITS);
      ...
      if (ops->ops_color_info_present_flag) {
        read_ops_color_info(&op->col_info, rb);
      } else {
        op->col_info.ops_color_description_idc = 0;
        op->col_info.ops_color_primaries = AVM_CICP_CP_UNSPECIFIED;
        op->col_info.ops_transfer_characteristics = AVM_CICP_TC_UNSPECIFIED;
        op->col_info.ops_matrix_coefficients = AVM_CICP_CP_UNSPECIFIED;
        op->col_info.ops_full_range_flag = 0;
      }
      ...
      if (op->ops_initial_display_delay_present_flag) {
        op->ops_initial_display_delay_minus_1 = avm_rb_read_literal(rb, 4);
      }
