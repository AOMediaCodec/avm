...
static uint32_t read_ats_region_info(struct AV2Decoder *pbi,
...
#if CONFIG_ATLAS_UPDATE
  atlas_reg_params->ats_num_region_columns_minus1 = avm_rb_read_uvlc(rb);
  const int num_regions_column =
      atlas_reg_params->ats_num_region_columns_minus1 + 1;
  ...
  atlas_reg_params->ats_num_region_rows_minus1 = avm_rb_read_uvlc(rb);
  const int num_regions_row = atlas_reg_params->ats_num_region_rows_minus1 + 1;
  if (num_regions_row > MAX_ATLAS_REGIONS) {
    avm_internal_error(&pbi->common.error, AVM_CODEC_UNSUP_BITSTREAM,
                       "The value of ats_num_region_rows_minus_1 shall "
  ...
  if (!atlas_reg_params->ats_uniform_spacing_flag) {
    for (int i = 0; i < num_regions_column; i++) {
      atlas_reg_params->ats_column_width_minus1[i] = avm_rb_read_uvlc(rb);
      atlas_reg_params->AtlasWidth +=
          (atlas_reg_params->ats_column_width_minus1[i] + 1);
    }
    for (int i = 0; i < num_regions_row; i++) {
      atlas_reg_params->ats_row_height_minus1[i] = avm_rb_read_uvlc(rb);
      atlas_reg_params->AtlasHeight +=
          (atlas_reg_params->ats_row_height_minus1[i] + 1);
    }
  } else {
...
static uint32_t read_ats_basic_atlas_info(
    struct AV2Decoder *pbi, struct AtlasBasicAtlasInfo *ats_basic_info,
    struct avm_read_bit_buffer *rb)
#else
...
#endif  // CONFIG_ATLAS_UPDATE
{
  ...
  ats_basic_info->ats_num_atlas_segments_minus1 = avm_rb_read_uvlc(rb);

  ats_basic_info->AtlasWidth = ats_basic_info->ats_atlas_width;
  ats_basic_info->AtlasHeight = ats_basic_info->ats_atlas_height;

  int NumSegments = ats_basic_info->ats_num_atlas_segments_minus1 + 1;
  if (NumSegments > MAX_NUM_ATLAS_SEGMENTS) {
    avm_internal_error(&pbi->common.error, AVM_CODEC_UNSUP_BITSTREAM,
                       "The value of ats_num_atlas_segments_minus1 "
                       "shall be in the range of 0 to %d, inclusive.",
...
static uint32_t read_ats_region_to_segment_mapping(
    ...
    int obu_xLayer_id, int xAId,
#endif
    int NumRegionsInAtlas, struct avm_read_bit_buffer *rb) {
#if CONFIG_ATLAS_UPDATE
  ats_reg_seg_map->ats_single_region_per_atlas_segment_flag =
      avm_rb_read_bit(rb);

  if (!ats_reg_seg_map->ats_single_region_per_atlas_segment_flag) {
    ats_reg_seg_map->ats_num_atlas_segments_minus1 = avm_rb_read_uvlc(rb);
    int NumSegments = ats_reg_seg_map->ats_num_atlas_segments_minus1 + 1;
    if (NumSegments > MAX_NUM_ATLAS_SEGMENTS) {
      avm_internal_error(&pbi->common.error, AVM_CODEC_UNSUP_BITSTREAM,
    ...
    for (int i = 0; i < NumSegments; i++) {
      ...
      ats_reg_seg_map->ats_bottom_right_region_column_off[i] =
          avm_rb_read_uvlc(rb);
      ats_reg_seg_map->ats_bottom_right_region_row_off[i] =
          avm_rb_read_uvlc(rb);
    ...
    if (NumRegionsInAtlas > MAX_NUM_ATLAS_SEGMENTS) {
      avm_internal_error(
          ...
          "If ats_single_region_per_atlas_segment_flag is enabled, "
          "NumRegionsInAtlas shall not be larger than %d.",
          MAX_NUM_ATLAS_SEGMENTS);
    }
    ats_reg_seg_map->ats_num_atlas_segments_minus1 = NumRegionsInAtlas - 1;
  }
#else
  ats_reg_seg_map
...
#if CONFIG_ATLAS_UPDATE
  ats_label->ats_signaled_atlas_segment_ids_flag = avm_rb_read_bit(rb);
  if (ats_label->ats_signaled_atlas_segment_ids_flag) {
    for (int i = 0; i < NumSegments; i++) {
...
static uint32_t read_ats_multistream_atlas_info(
    ...
    struct AtlasMultistreamAtlasInfo *ats_multistream_info, int hasAlpha,
    struct avm_read_bit_buffer *rb)
#else
...
{
#if CONFIG_ATLAS_UPDATE
  ats_multistream_info->ats_atlas_width = avm_rb_read_uvlc(rb);
  ats_multistream_info->ats_atlas_height = avm_rb_read_uvlc(rb);
  ats_multistream_info->ats_num_atlas_segments_minus1 = avm_rb_read_uvlc(rb);

  int NumSegments = ats_multistream_info->ats_num_atlas_segments_minus1 + 1;
  if (NumSegments > MAX_NUM_ATLAS_SEGMENTS) {
    avm_internal_error(&pbi->common.error, AVM_CODEC_UNSUP_BITSTREAM,
                       "The value of ats_num_atlas_segments_minus1 "
                       "shall be in the range of 0 to %d, inclusive.",
  ...
  ats_multistream_info->AtlasWidth = ats_multistream_info->ats_atlas_width;
  ats_multistream_info->AtlasHeight = ats_multistream_info->ats_atlas_height;

  // Handle hasAlpha parameter as per updated spec
  if (hasAlpha) {
    ats_multistream_info->ats_alpha_segments_present_flag = avm_rb_read_bit(rb);
  } else {
    ats_multistream_info->ats_alpha_segments_present_flag = 0;
  }

  ats_multistream_info->ats_background_info_present_flag = avm_rb_read_bit(rb);
  if (ats_multistream_info->ats_background_info_present_flag == 1) {
    ats_multistream_info->ats_background_red_value = avm_rb_read_literal(rb, 8);
    ats_multistream_info->ats_background_green_value =
        avm_rb_read_literal(rb, 8);
    ats_multistream_info->ats_background_blue_value =
        avm_rb_read_literal(rb, 8);
  }

  for (int i = 0; i < NumSegments; i++) {
    ats_multistream_info->ats_input_stream_id[i] = avm_rb_read_literal(rb, 5);
    ats_multistream_info->ats_segment_top_left_pos_x[i] = avm_rb_read_uvlc(rb);
    ats_multistream_info->ats_segment_top_left_pos_y[i] = avm_rb_read_uvlc(rb);
    ats_multistream_info->ats_segment_width[i] = avm_rb_read_uvlc(rb);
    ats_multistream_info->ats_segment_height[i] = avm_rb_read_uvlc(rb);

    // Updated condition: i != ats_num_atlas_segments_minus1
    if (ats_multistream_info->ats_alpha_segments_present_flag == 1 &&
        i != ats_multistream_info->ats_num_atlas_segments_minus1) {
      ats_multistream_info->ats_alpha_segment_flag[i] = avm_rb_read_bit(rb);
    } else {
      ats_multistream_info->ats_alpha_segment_flag[i] = 0;
    }
  }
#else
  ats_basic_info->ats_atlas_width[obu_xLayer_id][xAId] = avm_rb_read_uvlc(rb);
...
uint32_t av2_read_atlas_segment_info_obu(struct AV2Decoder *pbi,
...
#if CONFIG_ATLAS_UPDATE
  struct AtlasSegmentInfo *atlas_params =
      &pbi->atlas_list[obu_xLayer_id][atlas_segment_id];
  ...
  if (atlas_params->atlas_segment_mode_idc == ENHANCED_ATLAS) {
    read_ats_region_info(pbi, &atlas_params->ats_region_info, rb);
    read_ats_region_to_segment_mapping(
        pbi, &atlas_params->ats_region_to_segment_mapping,
        atlas_params->ats_region_info.NumRegionsInAtlas, rb);
    num_segments = atlas_params->ats_region_to_segment_mapping
                       .ats_num_atlas_segments_minus1 +
                   1;
  } else if (atlas_params->atlas_segment_mode_idc == BASIC_ATLAS) {
    read_ats_basic_atlas_info(pbi, &atlas_params->ats_basic_atlas_info, rb);
    num_segments =
        atlas_params->ats_basic_atlas_info.ats_num_atlas_segments_minus1 + 1;
  } else if (atlas_params->atlas_segment_mode_idc == SINGLE_ATLAS) {
    atlas_params->ats_region_to_segment_mapping.ats_num_atlas_segments_minus1 =
        0;
    ...
    read_ats_multistream_atlas_info(
        pbi, &atlas_params->ats_multistream_atlas_info, 0, rb);
    num_segments =
        atlas_params->ats_multistream_atlas_info.ats_num_atlas_segments_minus1 +
        1;
  } else if (atlas_params->atlas_segment_mode_idc == MULTISTREAM_ALPHA_ATLAS) {
    read_ats_multistream_atlas_info(
        pbi, &atlas_params->ats_multistream_atlas_info, 1, rb);
    num_segments =
        atlas_params->ats_multistream_atlas_info.ats_num_atlas_segments_minus1 +
        1;
  }
  // TODO: what happens if num_segments = -1?
  //  Label each atlas segment
  read_ats_label_segment_info(&atlas_params->ats_label_segment_info,
                              num_segments, rb);
#else
