...
static INLINE void debug_print_multiview_curr_frame(const AV1_COMMON *const cm) {
  const CurrentFrame *const cf = &cm->current_frame;
  const char frame_type_str[4][25] = {"KEY_FRAME", "INTER_FRAME", "INTRA_ONLY_FRAME", "S_FRAME"};
  const char ref_mode_str[4][50] = {"SINGLE_REFERENCE", "COMPOUND_REFERENCE", "REFERENCE_MODE_SELECT", "REFERENCE_MODES"};
...
static INLINE void debug_print_buffer_state(const AV1_COMMON *const cm) {
  ...
  for (int ref_idx = 0; ref_idx < REF_FRAMES; ref_idx++) {
    ...
    if (buf == NULL)
      ...
      printf("%d:(%d,%d,%d) ", ref_idx, buf->view_id, buf->pyramid_level, buf->display_order_hint);
  }
  printf("]\n");
}
...
#if CONFIG_MULTIVIEW_DEBUG_LOGFILES
static INLINE void logfile_multiview_curr_frame(const AV1_COMMON *const cm, FILE *const file) {
  const CurrentFrame *const cf = &cm->current_frame;
  const char frame_type_str[4][25] = {"KEY_FRAME", "INTER_FRAME", "INTRA_ONLY_FRAME", "S_FRAME"};
  const char ref_mode_str[4][50] = {"SINGLE_REFERENCE", "COMPOUND_REFERENCE", "REFERENCE_MODE_SELECT", "REFERENCE_MODES"};
...
static INLINE void logfile_multiview_buf_refs(const AV1_COMMON *const cm, FILE *const file) {
  const RefCntBuffer *const cf = cm->cur_frame;
  MV_REFERENCE_FRAME ref_frame;
  fprintf(file,"(View,OH,DOH):(%2d,%2d,%2d) ", cf->view_id ,cf->order_hint, cf->display_order_hint);
  fprintf(file,"  num_ref_frames/ref_frames_info.num_total_refs=%d/%d ", cf->num_ref_frames, cm->ref_frames_info.num_total_refs);
  fprintf(file,"[");
  for (ref_frame = 0; ref_frame < INTER_REFS_PER_FRAME; ++ref_frame) {
    const int map_idx = get_ref_frame_map_idx(cm, ref_frame);
...
static INLINE void logfile_buffer_state(const AV1_COMMON *const cm, FILE *const file) {
  fprintf(file," [ ");
  for (int ref_idx = 0; ref_idx < REF_FRAMES; ref_idx++) {
    RefCntBuffer *const buf = cm->ref_frame_map[ref_idx];
    if (buf == NULL)
      ...
      fprintf(file,"%d:(%d,%d,%d) ", ref_idx, buf->view_id, buf->order_hint, buf->display_order_hint);
  }
  fprintf(file,"]\n");
}

static INLINE void logfile_primary_ref_info(const AV1_COMMON *const cm, FILE *const file) {
  ...
  if (ref_buf == NULL)
    fprintf(file," Primary Reference Info. : [idx,map_idx]:[%2d,%2d] --> (N/A,N/A,N/A) \n ", idx, map_idx );
  else
    fprintf(file," Primary Reference Info. : [idx,map_idx]:[%2d,%2d] --> (%2d,%2d,%3d) \n ", idx, map_idx, ref_buf->view_id, ref_buf->order_hint, ref_buf->display_order_hint );
}

//static INLINE void logfile_show_existing_frame(const AV1_COMMON *const cm, FILE *const file) {
//  int show_existing_frame = cm->show_existing_frame;
//  if (show_existing_frame)
//    fprintf(file," Show existing frame : [idx,map_idx]:[%2d,%2d] --> (%2d,%2d,%3d) \n ", idx, map_idx, ref_buf->view_id, ref_buf->order_hint, ref_buf->display_order_hint );
//  
//  RefCntBuffer *const frame_to_show = cm->ref_frame_map[cpi->existing_fb_idx_to_show];
//}
#endif
