...
// check if the DMVR mode is allwed for a given mode
...
static INLINE int use_refine_mvs_for_mvp(const AV1_COMMON *const cm,
  ...
  int is_refinemv_supported =
      (mbmi->skip_mode
           ? is_refinemv_allowed_skip_mode(cm, mbmi)
           : is_refinemv_allowed(cm, mbmi, mbmi->sb_type[PLANE_TYPE_Y]));
  return (is_refinemv_supported && !is_intrabc_block(mbmi, xd->tree_type) &&
...
#if CONFIG_REFINEMV && USE_DEFAULT_SHARP_INTERPOLATION_FILTER
  // No interpolation filter search when DMVR MV refinement is used.
