...
static aom_codec_err_t decoder_peek_si_internal(const uint8_t *data,
  ...
  while (1) {
    ...
    if (obu_header.type == OBU_SEQUENCE_HEADER) {
      ...
      if (conf_win_flag) {
        int conf_win_left_offset = aom_rb_read_uvlc(&rb);
        int conf_win_right_offset = aom_rb_read_uvlc(&rb);
        int conf_win_top_offset = aom_rb_read_uvlc(&rb);
        int conf_win_bottom_offset = aom_rb_read_uvlc(&rb);
      }
