...
static int read_segment_id(AV1_COMMON *const cm, const MACROBLOCKD *const xd,
  ...
  uint8_t cdf_split_flag = 0;

  if (seg->enable_ext_seg == 1) {
    aom_cdf_prob *seg_cdf_split_flag_cdf =
        segp->seg_cdf_split_flag_cdf[cdf_num];
    cdf_split_flag = aom_read_symbol(r, seg_cdf_split_flag_cdf, 2,
                                     ACCT_INFO("seg_cdf_split_flag"));
  ...
  coded_id += cdf_split_flag ? MAX_SEGMENTS_8 : 0;
#else
