...
#else
static INLINE void read_vq_mvd(aom_reader *r, MV *mv_diff, int skip_sign_coding,
...
#endif
#if CONFIG_VQ_MVD_CODING
    read_vq_mvd(r, &mv_diff, 1, &ec_ctx->ndvc, MV_PRECISION_ONE_PEL, 0
#if !CONFIG_DERIVED_MVD_SIGN
                ,
...
static void read_truncated_unary_mvd(aom_reader *r, nmv_context *ctx,
  ...
  for (int bit_idx = 0; bit_idx < max_idx_bits; ++bit_idx) {
    ...
    // int this_bit = aom_read_literal(r, 1, ACCT_INFO());
...
static void read_tu_quasi_uniform(aom_reader *r, nmv_context *ctx,
  ...
  if (max_coded_value > max_trunc_unary_value && col == max_trunc_unary_value) {
    ...
    int remainder =
        aom_read_primitive_quniform(r, remainder_max_value + 1, ACCT_INFO_NAME);
  ...
  if (shell_class < 2) {
    ...
    for (int i = 0; i < num_of_bits_for_this_offset; ++i) {
      shell_cls_offset |=
          aom_read_symbol(
              r,
              ctx->shell_offset_other_class_cdf[get_class_offset_ctx(
                  precision, shell_class)][i],
              2, ACCT_INFO("offset"))
          << i;
    }
  }
  ...
  switch (mode) {
...
#endif  // CONFIG_DERIVED_MVD_SIGN
#if CONFIG_VQ_MVD_CODING
      read_vq_mvd(r, &mv_diff[0], 1, nmvc, precision, is_adaptive_mvd
#if !CONFIG_DERIVED_MVD_SIGN
                  ,
...
#endif  // CONFIG_OPTFLOW_REFINEMENT
    {
      ...
      for (int i = 0; i < 2; ++i) {
        nmv_context *const nmvc = &ec_ctx->nmvc;
#if CONFIG_VQ_MVD_CODING
        read_vq_mvd(r, &mv_diff[i], 1, nmvc, precision, is_adaptive_mvd
#if !CONFIG_DERIVED_MVD_SIGN
                    ,
...
#if CONFIG_VQ_MVD_CODING
      read_vq_mvd(r, &mv_diff[1], 1, nmvc, precision, is_adaptive_mvd
#if !CONFIG_DERIVED_MVD_SIGN
                  ,
...
#if CONFIG_VQ_MVD_CODING
      read_vq_mvd(
          r, &mv_diff[jmvd_base_ref_list], 1, nmvc, precision, is_adaptive_mvd
