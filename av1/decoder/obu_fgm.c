...
#define ERROR_NUM_POINT_Y 1
#define ERROR_FIRST_CORDINATE_SCALINNG_Y 2
#define ERROR_NUM_POINT_CB 3
#define ERROR_FIRST_CORDINATE_SCALINNG_CB 4
#define ERROR_NUM_POINT_CR 5
#define ERROR_FIRST_CORDINATE_SCALINNG_CR 6
#define ERROR_420_NUM_APPLIED_COMPONENT 7

static int read_film_grain_model(struct film_grain_model *fgm, int chroma_idc,
  ...
  // Initialization
  for (int i = 0; i < 3; i++) fgm->fgm_points[i] = 0;
  ...
  for (int c = 0; c < fgmNumScalingChannels; c++) {
    ...
    if (c == 0 && fgm->fgm_points[c] > 14) {
      return ERROR_NUM_POINT_Y;
    } else if (c > 0 && fgm->fgm_points[c] > 10) {
      return c == 1 ? ERROR_NUM_POINT_CB : ERROR_NUM_POINT_CR;
    }
    if (fgm->fgm_points[c]) {
      ...
      for (int i = 0; i < fgm->fgm_points[c]; i++) {
        ...
        if (i && fgm->fgm_scaling_points[c][i - 1][0] >=
                     fgm->fgm_scaling_points[c][i][0]) {
          return c == 0   ? ERROR_FIRST_CORDINATE_SCALINNG_Y
                 : c == 1 ? ERROR_FIRST_CORDINATE_SCALINNG_CB
                          : ERROR_FIRST_CORDINATE_SCALINNG_CR;
        }
  ...
  if ((subsampling_x == 1) && (subsampling_y == 1) &&
      (((fgm->fgm_points[1] == 0) && (fgm->fgm_points[2] != 0)) ||
       ((fgm->fgm_points[1] != 0) && (fgm->fgm_points[2] == 0)))) {
    return ERROR_420_NUM_APPLIED_COMPONENT;
  }
  ...
  if (fgm->num_y_points > 14) {
    ...
    //                       "Number of points for film grain luma scaling
  ...
  if (monochrome || fgm->chroma_scaling_from_luma ||
    ...
    if (fgm->num_cb_points > 10) {
      //      aom_internal_error(&cm->error, AOM_CODEC_UNSUP_BITSTREAM,
      //                         "Number of points for film grain cb scaling
      //                         function " "exceeds the maximum value.");
      return 3;
    }
    ...
    if (fgm->num_cr_points > 10) {
      return 5;
      //      aom_internal_error(&cm->error, AOM_CODEC_UNSUP_BITSTREAM,
      //                         "Number of points for film grain cr scaling
      //                         function " "exceeds the maximum value.");
    }
