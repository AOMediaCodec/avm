...
uint32_t read_buffer_timing_removal_obu(struct AV1Decoder *pbi,
  ...
  int br_decoder_model_present_op_flag;
  for (int i= 0 ; i < ops_cnt; i++) {
    btr_params->br_decoder_model_present_op_flag[xlayer_id][ops_id][i] = aom_rb_read_bit(rb);
    ...
    if (br_decoder_model_present_op_flag)
      btr_params->br_buffer_removal_time[xlayer_id][ops_id][i] = aom_rb_read_uvlc(rb);
  }
