...
int read_lcr_xlayer_color_info(int isGlobal, int xId) {
#if CONFIG_MULTILAYER_HLS_REMOVE_LOGS
  (void)isGlobal;
  (void)xId;
#else
  printf("read_lcr_xlayer_color_info(isGlobal=%d,xId=%d): not defined yet.\n",
         isGlobal, xId);
#endif  // CONFIG_MULTILAYER_HLS_REMOVE_LOGS
  return 0;
}
// TODO: (@hegilmez) to be implemented
int read_obu_extension_bits(struct ObuExtension *ext_params,
                            struct aom_read_bit_buffer *rb) {
  (void)ext_params;
  (void)rb;
#if !CONFIG_MULTILAYER_HLS_REMOVE_LOGS
  printf("read_obu_extension_bits(): not defined yet.\n");
#endif  // !CONFIG_MULTILAYER_HLS_REMOVE_LOGS
  return 0;
}
  ...
  for (int j = 0; j < 8; j++) {
    if ((mlayer_params->lcr_mlayer_map[isGlobal][xId] & (1 << j))) {
      ...
      if (atlasSegmentPresent) {
        ...
        mlayer_params->lcr_overlay_method[isGlobal][xId][j] =
            aom_rb_read_literal(rb, 8);
      ...
      if (!crop_params->crop_info_in_scr_flag) {
        ...
        crop_params->crop_win_left_offset = aom_rb_read_uvlc(rb);
        crop_params->crop_win_right_offset = aom_rb_read_uvlc(rb);
        crop_params->crop_win_top_offset = aom_rb_read_uvlc(rb);
        crop_params->crop_win_bottom_offset = aom_rb_read_uvlc(rb);
      }
...
int read_lcr_xlayer_info(struct AV1Decoder *pbi, int isGlobal, int xId,
  ...
  if (lcr_params->lcr_xlayer_color_info_present_flag[isGlobal][xId])
    read_lcr_xlayer_color_info(isGlobal, xId);
...
void read_lcr_global_payload(struct AV1Decoder *pbi, int i, int sizePresent,
                             struct aom_read_bit_buffer *rb) {
  struct LayerConfigurationRecord lcr_params = pbi->common.lcr_params;
  struct ObuExtension lcr_extension = lcr_params.lcr_extension;
  ...
  if (sizePresent) {
    lcr_extension.extension_present_flag =
        1;  // Set this to 1 if threre is an extension
    lcr_extension.extension_data =
        aom_rb_read_literal(rb, 1);  // TODO: (@hegilmez) - le(n)
  }
}

uint32_t read_data_size(struct aom_read_bit_buffer *rb) {
  int length_field_size = 0;
  int temp = 0;
  uint32_t size = 0;

  if (aom_uleb_decode(rb->bit_offset, 0, temp, length_field_size) != 0) {
...
int read_lcr_global_info(struct AV1Decoder *pbi,
  ...
  int lcr_pos = -1;
  for (int i = 0; i < pbi->dec_lcr_counter; i++) {
  ...
  ObuExtension lcr_extension = lcr_params->lcr_extension;
  ...
  for (int i = 0; i < lcr_params->lcr_max_num_extended_layers_minus_1 + 1;
    ...
    if (lcr_params->lcr_data_size_present_flag)
      lcr_params->lcr_data_size[i] = read_data_size(rb);
    read_lcr_global_payload(pbi, i, lcr_params->lcr_data_size_present_flag, rb);
  }

  lcr_extension.extension_present_flag = aom_rb_read_bit(rb);
  if (lcr_extension.extension_present_flag)
    read_obu_extension_bits(&lcr_params->lcr_extension, rb);
