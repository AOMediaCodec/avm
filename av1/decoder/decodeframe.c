  ...
  if (
    ...
    for (int i = 0; i < seq_params->ref_frames; i++) {
      if (is_frame_eligible_for_output(cm->ref_frame_map[i]) &&
        ...
        aom_internal_error(
            &cm->error, AOM_CODEC_UNSUP_BITSTREAM,
            "If enable_frame_output_order is equal to 1, frame_type is not "
            "equal to KEY_FRAME and is_frame_eligible_for_output(i) is equal "
            "to 1, it is a requirement of bitstream conformance that the value "
            "returned from get_disp_order_hint is not equal to RefOrderHint[ i "
            "] for any i in the range 0..NumRefFrames-1.");
      }
    }
  }
#endif
