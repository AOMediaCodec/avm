...
#if CONFIG_MIXED_LOSSLESS_ENCODE && CHECK_LOSSLESS_PSNR && \
    FIXED_MIXED_LOSSLESS_PATTERN
...
static void check_mixed_lossless_plane(AV1_COMP *cpi, const uint16_t *a,
  ...
  int row_th =
      (NUMBER_OF_LOSSY_SBS_ROWS * (cpi->common.mib_size << MI_SIZE_LOG2)) >>
      shift;
  int col_th =
      (NUMBER_OF_LOSSY_SBS_COLS * (cpi->common.mib_size << MI_SIZE_LOG2)) >>
      shift;
  printf(" plane = %d row_th = %d , col_th = %d \n", plane, row_th, col_th);
#endif
  ...
  for (int y = 0; y < height; ++y) {
    for (int x = 0; x < width; ++x) {
...
#if FIXED_MIXED_LOSSLESS_PATTERN == 2
...
#elif FIXED_MIXED_LOSSLESS_PATTERN == 1
      int v1 = (x < col_th);
      int v2 = (y < row_th);
      int is_lossless = v1 ^ v2;
#endif
      ...
      CHECK_LOSSLESS(!IMPLIES(is_lossless, org == rec),
                     " target lossless block has non-zero error");
    }
  }
}
...
#endif  // CONFIG_MIXED_LOSSLESS_ENCODE && CHECK_LOSSLESS_PSNR &&
        // FIXED_MIXED_LOSSLESS_PATTERN
...
#if CONFIG_CHECK_LOSSLESS_MIXED_CONTENT
...
#endif  // CONFIG_CHECK_LOSSLESS_MIXED_CONTENT
      }
    }
    if (is_frame_visible) {
