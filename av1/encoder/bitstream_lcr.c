...
int write_lcr_xlayer_color_info(int isGlobal, int xId) {
#if CONFIG_MULTILAYER_HLS_REMOVE_LOGS
  (void)isGlobal;
  (void)xId;
#else
  printf(
      "write_lcr_xlayer_color_info(isGlobal=%d,xId=%d): profile, tier, level "
      "definitions not defined yet\n",
      isGlobal, xId);
#endif  // CONFIG_MULTILAYER_HLS_REMOVE_LOGS
  return 0;
}
...
int write_obu_extension_bits(struct ObuExtension *ext_params,
  ...
  (void)wb;
#if !CONFIG_MULTILAYER_HLS_REMOVE_LOGS
  printf("write_obu_extension_bits(): not defined yet.\n");
#endif  // !CONFIG_MULTILAYER_HLS_REMOVE_LOGS
  return 0;
}
...
int write_lcr_embedded_layer_info(AV1_COMP *cpi, int isGlobal, int xId,
  ...
  aom_wb_write_literal(wb, mlayer_params->lcr_mlayer_map[isGlobal][xId], 8);
  ...
  for (int j = 0; j < 8; j++) {
    if ((mlayer_params->lcr_mlayer_map[isGlobal][xId] & (1 << j))) {
      ...
      if (atlasSegmentPresent) {
        ...
        aom_wb_write_literal(
            wb, mlayer_params->lcr_overlay_method[isGlobal][xId][j], 8);
        aom_wb_write_literal(
      ...
      if (!crop_params->crop_info_in_scr_flag) {
        ...
        aom_wb_write_uvlc(wb, crop_params->crop_win_left_offset);
        aom_wb_write_uvlc(wb, crop_params->crop_win_right_offset);
        aom_wb_write_uvlc(wb, crop_params->crop_win_top_offset);
        aom_wb_write_uvlc(wb, crop_params->crop_win_bottom_offset);
      }
    }
  }
...
int write_lcr_xlayer_info(AV1_COMP *cpi, int isGlobal, int xId,
  ...
  if (lcr_params->lcr_xlayer_color_info_present_flag[isGlobal][xId])
    write_lcr_xlayer_color_info(isGlobal, xId);
...
void write_lcr_global_payload(AV1_COMP *cpi, int i, int sizePresent,
                              struct aom_write_bit_buffer *wb) {
  struct LayerConfigurationRecord lcr_params = cpi->common.lcr_params;
  struct ObuExtension ext_params = lcr_params.lcr_extension;
  ...
  if (sizePresent)
    aom_wb_write_literal(wb, ext_params.extension_data,
...
uint32_t write_layer_configuration_record_obsp(
    AV1_COMP *cpi, LayerConfigurationRecord *lcr_params, int xlayer_id,
    uint8_t *dst) {
  struct aom_write_bit_buffer wb = { dst, 0 };
  uint32_t size = 0;

  ObuExtension *ext_params = &lcr_params->lcr_extension;
  ...
  aom_wb_write_bit(&wb, ext_params->extension_present_flag);
  if (ext_params->extension_present_flag)
    write_obu_extension_bits(&lcr_params->lcr_extension, &wb);
