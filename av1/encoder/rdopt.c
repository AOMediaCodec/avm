...
static int cost_mv_ref(const ModeCosts *const mode_costs, PREDICTION_MODE mode,
  ...
  if (is_inter_compound_mode(mode)) {
    ...
    if (not_allow_new_nearmv_pred_mode(mbmi)) {
...
#if CONFIG_OPT_INTER_MODE_CTX
static INLINE int not_allow_compound_mode(PREDICTION_MODE mode, int ref_frame0,
                                          int ref_frame1) {
...
#if CONFIG_OPT_INTER_MODE_CTX
             not_allow_compound_mode(this_mode, ref_frame, second_ref_frame) ||
#endif  // CONFIG_OPT_INTER_MODE_CTX
             ((second_ref_frame == ref_frame) &&
