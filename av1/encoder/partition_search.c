...
#if CONFIG_CWG_F317
  if (partition_found &&
    ((!bru_is_sb_active(&cpi->common, x->e_mbd.mi_col, x->e_mbd.mi_row)) || cpi->common.bridge_frame_info.is_bridge_frame)) {
#else
  if (partition_found &&
...
#if CONFIG_CWG_F317
  if ((!bru_is_sb_active(cm, blk_params.mi_col, blk_params.mi_row)) || cm->bridge_frame_info.is_bridge_frame) {
#else
...
#if CONFIG_CWG_F317
  if (part_search_state->found_best_partition && ((!bru_is_sb_active(cm, mi_col, mi_row)) || cm->bridge_frame_info.is_bridge_frame)) {
#else
