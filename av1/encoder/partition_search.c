...
static void update_partition_stats(MACROBLOCKD *const xd,
  ...
  const PARTITION_TYPE derived_partition =
      av1_get_normative_forced_partition_type(
          ...
          true,           // as it is a partition point
...
static void encode_sb(const AV1_COMP *const cpi, ThreadData *td,
  ...
  const int is_partition_root = is_partition_point(
      bsize
#if CONFIG_CB1TO4_SPLIT
      ,
...
static AOM_INLINE void init_allowed_partitions(
  ...
  part_search_state->is_block_splittable = is_partition_point(
      bsize
#if CONFIG_CB1TO4_SPLIT
      ,
      blk_params->parent_bsize, enable_unrestricted_cb1to4_partitioning
#endif  // CONFIG_CB1TO4_SPLIT
  );
...
static void init_partition_search_state_params(
  ...
  init_allowed_partitions(
      part_search_state, &cpi->oxcf.part_cfg, &pc_tree->chroma_ref_info,
#if CONFIG_CB1TO4_SPLIT
      cm->seq_params.enable_unrestricted_cb1to4_partitioning,
#endif  // CONFIG_CB1TO4_SPLIT
      tree_type);
...
bool av1_rd_pick_partition(AV1_COMP *const cpi, ThreadData *td,
  ...
  if (counterpart_block
      ...
      &&
      (is_partition_point(
           bsize, pc_tree->parent ? pc_tree->parent->block_size : BLOCK_INVALID,
           cm->seq_params.enable_unrestricted_cb1to4_partitioning) ==
       is_partition_point(
           counterpart_block->block_size,
           counterpart_block->parent ? counterpart_block->parent->block_size
                                     : BLOCK_INVALID,
           cm->seq_params.enable_unrestricted_cb1to4_partitioning))
#endif  // CONFIG_CB1TO4_SPLIT
#if CONFIG_EXTENDED_SDP
  ...
  if (part_search_state.forced_partition == PARTITION_INVALID) {
    if (cpi->sf.part_sf.adaptive_partition_search_order) {
      search_none_after_rect = try_none_after_rect(
          xd, &cm->mi_params, bsize,
#if CONFIG_CB1TO4_SPLIT
          blk_params.parent_bsize,
          cm->seq_params.enable_unrestricted_cb1to4_partitioning,
#endif  // CONFIG_CB1TO4_SPLIT
...
#if CONFIG_CB1TO4_SPLIT
      if (cm->seq_params.enable_unrestricted_cb1to4_partitioning ||
          pc_tree->parent == NULL ||
