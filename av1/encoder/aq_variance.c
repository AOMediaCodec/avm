  ...
  if (frame_is_intra_only(cm) || cm->features.error_resilient_mode) {
    ...
    for (i = 0; i < max_seg_num; ++i) {
      ...
      if (i > 0) {
        ...
        qindex_delta = 0;  // 210 - base_qindex;
      }
...
int av1_log_block_var(const AV1_COMP *cpi, MACROBLOCK *x, BLOCK_SIZE bs
#if CONFIG_MIXED_LOSSLESS_ENCODE && FIXED_MIXED_LOSSLESS_PATTERN
                      ,
                      int mi_row, int mi_col
#endif  // CONFIG_MIXED_LOSSLESS_ENCODE && FIXED_MIXED_LOSSLESS_PATTERN
) {
  ...
  return lossless;
#else
  int mi_row_th = NUMBER_OF_LOSSY_SBS_ROWS * cpi->common.mib_size;
  int mi_col_th = NUMBER_OF_LOSSY_SBS_COLS * cpi->common.mib_size;
  int a = mi_row < mi_row_th;
  int b = mi_col < mi_col_th;
  return (a ^ b);
#endif  //
