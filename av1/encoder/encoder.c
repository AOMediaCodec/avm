...
#if CONFIG_F255_QMOBU
  //[jkei] quant_params.using_qmatrix is not set
  // 15 cm->quant_params is initialized with predefined matrices
  av1_qm_init(&cm->quant_params, av1_num_planes(cm));
#if CONFIG_F255_QMOBU_USERQM_TEST
  const QuantizationCfg *const q_cfg = &oxcf->q_cfg;
  bool user_defined_qmatrix = q_cfg->using_qm && q_cfg->user_defined_qmatrix;
  if (user_defined_qmatrix) {
    //[jkei] load user defindex qmatrix to cpi->user_defined_qm_list
    av1_test_user_defined_qm(cpi);
  }  // user_defined_qmatrix
