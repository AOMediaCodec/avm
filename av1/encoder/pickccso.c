 ...
 */
 ...
 #if CCSO_REFACTORING
   const int unit_log2_x = proc_unit_log2 > blk_log2_x ? blk_log2_x : proc_unit_log2;
   const int unit_size_x = 1 << (unit_log2_x);
   const int unit_log2_y = proc_unit_log2 > blk_log2_y ? blk_log2_y : proc_unit_log2;
         ...
         for (int unit_x = 0; unit_x < x_end; unit_x += unit_size_x) {
           ...
           if (ccso_bo_only) {
             ...
             ccso_filter_block_hbd_with_buf_c(
                 ...
                 unit_size_x,
 #endif  // CONFIG_CCSO_FU_BUGFIX
                 unit_size_y, y_uv_hscale, y_uv_vscale,
