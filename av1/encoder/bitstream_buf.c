...
void set_btr_params(AV1_COMP *const cpi) {
  ...
  BufferTimingRemoval *btr_params =  &cpi->common.btr_params;
  
  //TODO: @hilmi: please connect these
  /*
   btr_params->obu_xlayer_id == obu xlayer id
   btr_params->br_ops_id[xlayer_id] = ops_id[obu_xlayer_id]
   btr_params->br_ops_cnt[xlayer_id][ops_id] = ops_cnt[obu_xlayer_id][ops_id]
   */
  btr_params->obu_xlayer_id = 0;
  int xlayer_id = btr_params->obu_xlayer_id;
  btr_params->br_ops_id[xlayer_id] =  0;
  int ops_id = btr_params->br_ops_id[xlayer_id];
  btr_params->br_ops_cnt[xlayer_id][ops_id] = 0;
  ...
  for (int i = 0; i < ops_cnt; i++) {
    ...
    br_decoder_model_present_op_flag = btr_params->br_decoder_model_present_op_flag[xlayer_id][ops_id][i];
...
uint32_t av2_write_buffer_timing_removal_obu(const BufferTimingRemoval
  ...
  for (int i= 0 ; i < ops_cnt; i++) {
    aom_wb_write_bit(&wb, btr_params->br_decoder_model_present_op_flag[xlayer_id][ops_id][i]);
    if (btr_params->br_decoder_model_present_op_flag[xlayer_id][ops_id][i])
      aom_wb_write_uvlc(&wb, btr_params->br_buffer_removal_time[xlayer_id][ops_id][i]);
  }
