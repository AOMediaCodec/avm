...
#if !CONFIG_EXTRA_DPB
  const int ref_frames = REF_FRAMES;
#endif
#if CONFIG_MULTILAYER_ENCODER_TEMPORAL_SCALABILITY || \
    CONFIG_MULTILAYER_ENCODER_SEPARATE_DPB
  int start_idx = 0;
...
#if CONFIG_MULTILAYER_ENCODER_SEPARATE_DPB
...
static int get_refresh_frame_flags_subgop_cfg(
  ...
  printf("In (subgop)  - get_refresh_idx: (View,Level,OH,DOH):(%d,%d,%d,%d) \n",
         cf->view_id, cf->pyramid_level, cf->order_hint,
         cf->display_order_hint);
  printf("Out (subgop) - get_refresh_idx: %d \n", refresh_idx);
#endif
...
#if CONFIG_MULTILAYER_DEBUG_PROMPT
  printf(
      "In (getrff:1) - get_refresh_idx: (View,Level,OH,DOH):(%d,%d,%d,%d) \n",
      cf->view_id, cf->pyramid_level, cf->order_hint, cf->display_order_hint);
  printf("Out (getrff:1) - free_fb_index: %d \n", free_fb_index);
#endif
...
#if CONFIG_MULTILAYER_DEBUG_PROMPT
  printf(
      "In (getrff:2) - get_refresh_idx: (View,Level,OH,DOH):(%d,%d,%d,%d) \n",
      cf->view_id, cf->pyramid_level, cf->order_hint, cf->display_order_hint);
  printf("Out (getrff:2) get_refresh_idx: %d \n", refresh_idx);
#endif
  return 1 << refresh_idx;
}
...
#endif
#if CONFIG_MULTILAYER_CORE
        cm->current_frame.view_id, cm->number_layers,
#endif
#if CONFIG_PRIMARY_REF_FRAME_OPT
