...
static AOM_INLINE void write_segment_id(AV1_COMP *cpi,
  ...
  aom_cdf_prob *seg_cdf_split_flag_cdf = segp->seg_cdf_split_flag_cdf[cdf_num];

  if (seg->enable_ext_seg == 1) {
    if (coded_id < MAX_SEGMENTS_8) {
      aom_write_symbol(w, 0, seg_cdf_split_flag_cdf, 2);
      // use cdf0
      pred_cdf = segp->spatial_pred_seg_cdf[cdf_num];
    } else {
      aom_write_symbol(w, 1, seg_cdf_split_flag_cdf, 2);
      // use cdf1
