...
#else
      aom_wb_write_literal(wb, ref_idx, REF_FRAMES_LOG2);
#endif
    return;
  }
...
#if CONFIG_F159_OBU_HEADER
  const int obu_temporal =
      0;  //@hegilmez TODO: hook values for the temoporal_id
  const int obu_mlayer = cm->current_frame.view_id;
