...
const unsigned int kMidGopPosition = 5;

// This class tests that inserting an extra sequence header mid-GOP does not
// cause crashes or assertion failures in the encoder or decoder.
class AddSeqHeaderInGOPTest
    : public ::libavm_test::CodecTestWithParam<int>,
      public ::libavm_test::EncoderTest {
 protected:
  AddSeqHeaderInGOPTest()
      : EncoderTest(GET_PARAM(0)), cpu_used_(GET_PARAM(1)) {}

  ~AddSeqHeaderInGOPTest() override {}
  ...
  void PreEncodeFrameHook(::libavm_test::VideoSource *video,
    ...
    if (video->frame() == 0) {
      ...
      encoder->Control(AV2E_SET_ADD_SEQ_HEADER_IN_GOP_TEST, kMidGopPosition);
    }
  }
...
TEST_P(AddSeqHeaderInGOPTest, EndtoEndTest) {
  ::libavm_test::Y4mVideoSource video("park_joy_90p_8_420.y4m", 0, kFrames);
  ASSERT_NO_FATAL_FAILURE(RunLoop(&video));
}

AV2_INSTANTIATE_TEST_SUITE(AddSeqHeaderInGOPTest, ::testing::Values(kCpuUsed));
}  // namespace
