 ...
 public:
  ...
  void GetNzMapContextsRun() {
    ...
    for (int plane = 0; plane < 3; ++plane) {
      for (int is_inter = 0; is_inter < 2; ++is_inter) {
        for (int primary_tx_type = DCT_DCT; primary_tx_type < PRIMARY_TX_TYPES;
             ++primary_tx_type) {
          ...
          for (int tx_size = TX_4X4; tx_size < TX_SIZES_ALL; ++tx_size) {
            ...
            const int16_t *const scan =
                av2_scan_orders[tx_size][primary_tx_type].scan;
  ...
  void SpeedTestGetNzMapContextsRun() {
    ...
    for (int tx_size = TX_4X4; tx_size < TX_SIZES_ALL; ++tx_size) {
      ...
      const int real_width = tx_size_wide[tx_size];
      const int real_height = tx_size_high[tx_size];
      const PRIMARY_TX_TYPE primary_tx_type = DCT_DCT;
      const TX_CLASS tx_class = tx_type_to_class[primary_tx_type];
      const int16_t *const scan =
          av2_scan_orders[tx_size][primary_tx_type].scan;
