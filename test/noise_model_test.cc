...
#if CONFIG_CWG_F109
    EXPECT_EQ(test_case.expected_value, film_grain.fgm_value_scale[0][0]);
    EXPECT_EQ(test_case.expected_value, film_grain.fgm_value_scale[0][1]);
#else
...
TEST(NoiseModelGetGrainParameters, GetGrainParametersReal) {
  ...
  for (int i = 0; i < kNumScalingPointsY; ++i) {
    EXPECT_EQ(kExpectedScalingPointsY[i][0],
              film_grain.fgm_value_increment[0][i]);
    EXPECT_EQ(kExpectedScalingPointsY[i][1], film_grain.fgm_value_scale[0][i]);
  }
#else
  for (int i = 0; i < kNumScalingPointsY; ++i) {
  ...
  EXPECT_EQ(0, film_grain.fgm_value_increment[1][0]);
  EXPECT_EQ(255, film_grain.fgm_value_increment[1][1]);
  EXPECT_EQ(96, film_grain.fgm_value_scale[1][0]);
  EXPECT_EQ(96, film_grain.fgm_value_scale[1][1]);

  // CR strength should just be a piecewise segment
  EXPECT_EQ(2, film_grain.fgm_points[2]);
  EXPECT_EQ(0, film_grain.fgm_value_increment[2][0]);
  EXPECT_EQ(255, film_grain.fgm_value_increment[2][1]);
  EXPECT_EQ(64, film_grain.fgm_value_scale[2][0]);
  EXPECT_EQ(64, film_grain.fgm_value_scale[2][1]);
#else
