...
TEST_P(InvTxfmVariantTest, BitExact) {
  ...
  txfm_param.primary_tx_type = p.primary_tx_type;
  txfm_param.bd = p.bd;
  ...
  for (int y = 0; y < txh; y++) {
    for (int x = 0; x < txw; x++) {
      ASSERT_EQ(ref_dest[y * stride + x], opt_dest[y * stride + x])
          ...
          << " tx_type=" << p.primary_tx_type << " bd=" << p.bd
          << " seed=" << p.seed << " use_ddt=" << p.use_ddt;
    }
  }
}

static bool is_valid_inv_txfm_combo(TX_SIZE sz, PRIMARY_TX_TYPE ty) {
...
static std::vector<InvTxfmParam> GenerateParams() {
  ...
  const PRIMARY_TX_TYPE types[] = { DCT_DCT, ADST_DCT, DCT_ADST, ADST_ADST,
                                    IDTX };
