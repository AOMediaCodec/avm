...
from Config import (
    ...
    FFMPEG,
    HDRConvert,
    HDRToolsConfigFileTemplate,
    LoggerName,
    Platform,
)
from Utils import ExecuteCmd, GetShortContentName, md5
...
def RescaleWithFfmpeg(clip, outw, outh, algo, outfile, num, LogCmdOnly):
    ...
    args = (
        " -y -i %s -vf scale=%d:%d -pix_fmt %s -strict -1"
...
def RescaleWithHDRTool(
    clip, outw, outh, algo, outfile, num, cfg_path, LogCmdOnly=False
):
...
def VideoRescaling(
    method, clip, num, outw, outh, outfile, algo, cfg_path, LogCmdOnly=False
):
    if method == "hdrtool":
...
####################################################################################
##################### Major Functions ################################################
def GetDownScaledOutFile(
    clip, dnw, dnh, path, method, algo, ds_on_the_fly=True, ratio_idx=0
):
    contentBaseName = GetShortContentName(clip.file_name, False)
    ...
    if clip.width != dnw or clip.height != dnh:
        if ds_on_the_fly:
            filename = contentBaseName + (
                "_Scaled_%s_%s_%dx%d.y4m" % (method, algo, dnw, dnh)
            )
...
def GetUpScaledOutFile(clip, outw, outh, method, algo, path):
    ...
    if clip.width != outw or clip.height != outh:
        filename = contentBaseName + (
            "_Scaled_%s_%s_%dx%d.y4m" % (method, algo, outw, outh)
        )
...
def GetDownScaledMD5File(clip, dnw, dnh, path, method, algo):
    ...
    if clip.width != dnw or clip.height != dnh:
        filename = contentBaseName + (
            "_Scaled_%s_%s_%dx%d.md5" % (method, algo, dnw, dnh)
        )
...
def DownScaling(method, clip, num, outw, outh, path, cfg_path, algo, LogCmdOnly=False):
    ...
    if clip.width != outw or clip.height != outh:
        ...
        VideoRescaling(
            method, clip, num, outw, outh, dnScaledOut, algo, cfg_path, LogCmdOnly
        )
...
def UpScaling(method, clip, num, outw, outh, path, cfg_path, algo, LogCmdOnly=False):
    ...
    if clip.width != outw or clip.height != outh:
        ...
        VideoRescaling(
            method, clip, num, outw, outh, upScaleOut, algo, cfg_path, LogCmdOnly
        )
    return upScaleOut
