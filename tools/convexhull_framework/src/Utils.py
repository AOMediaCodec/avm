...
from AV2CTCVideo import CTC_TEST_SET, ECF_TEST_SET, ECF_Y4M_CLIPs, Y4M_CLIPs
from AV2SubjectiveVideo import AV2_SUBJECTIVE_TEST, SUBJECTIVE_CLIPS
from Config import (
    APSNR_U_WEIGHT,
    APSNR_V_WEIGHT,
    APSNR_Y_WEIGHT,
...
class Record:
    ...
    def __init__(
        ...
        self.overall_psnr = (
            psnr_yw * self.psnr_y + psnr_uw * self.psnr_u + psnr_vw * self.psnr_v
        ) / (psnr_yw + psnr_uw + psnr_vw)
        ...
        self.overall_apsnr = 10 * math.log10(
            1
            / (
                (
                    apsnr_yw / pow(10, (self.apsnr_y / 10))
    ...
    try:
        for line in flog:
            if line.startswith("POC"):
                ...
                m = re.search(
                    r"POC:\s+(\d+)\s+\[( KEY |INTER)\]\[Level:(\d+)\]\[Q:\s*(\d+)\]:\s+(\d+)\s+Bytes,",
                    line,
                )
                if m:
                    POC = m.group(1)
...
def Interpolate_Bilinear(RDPoints, QPs, InterpolatePieces, logBr=True):
    ...
    int_points = [
        (round(int_points[i][0], 6), round(int_points[i][1], 6))
...
if __name__ == "__main__":
    ...
    v100 = {
        "2160p": [
            (28124.3888, 42.551087),
            (13410.6054, 41.963043),
            (6624.8041, 41.253859),
            (3708.8954, 40.269257),
            (2267.0327, 38.851295),
            (1202.0951, 36.555146),
    ...
    for res in reslutions:
        ...
        rdpnts = [
            (ext_quant[res][i][0], ext_quant[res][i][1])
            for i in range(len(ext_quant[res]))
        ]
        if UsePCHIPInterpolation:
            int_rdpnts = Interpolate_PCHIP(
                rdpnts, QPs["AS"][:], InterpolatePieces, True
    ...
    plot_rd_curve(
        br["v100"], psnr["v100"], "psnr_y", "v1.0.0", "bitrate(Kbps)", "b", "-", "*"
    )
    plot_rd_curve(
        br["ext_quant"],
        psnr["ext_quant"],
    ...
    bdrate = BD_RATE(
        "psnr_y", br["v100"], psnr["v100"], br["ext_quant"], psnr["ext_quant"]
    )
    ...
    for res in reslutions:
        rdpnts = [
            (rdpoints[res][i][0], rdpoints[res][i][1])
            for i in range(len(rdpoints[res]))
        ]
