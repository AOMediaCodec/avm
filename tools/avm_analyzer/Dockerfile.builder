...
RUN apt-get update && apt-get install -y make pkg-config git unzip cmake protobuf-compiler libssl-dev --no-install-recommends
RUN cargo install trunk --locked --version ^0.20
...
COPY tools/avm_analyzer .
RUN cd avm_analyzer_app && trunk build --release
