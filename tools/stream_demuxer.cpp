...
static int read_multi_stream_HEADER(struct aom_read_bit_buffer *rb,
                                    int *stream_ids) {
#if PRINT_TU_INFO
...
#endif  // PRINT_TU_INFO
  if (multistream_level_idx > 63) {
    fprintf(stderr,
            "multistream_level_idx cannot exceed the max value (63).\n");
    return -1;
  }
...
#endif  // PRINT_TU_INFO
    if (substream_profile_idx > 3) {
      fprintf(stderr,
              "sub-stream_profile_idx[%d] cannot exceed the max value (3).\n",
              i);
      return -1;
    }
...
std::vector<uint8_t> ExtractTU(const uint8_t *data, int length,
  ...
  while (consumed < length) {
    ...
    *current_stream_id = obu_header.obu_mlayer_id;
    ...
    } else if (obu_header.type == OBU_MULTI_STREAM_HEADER) {
...
#endif  // CONFIG_F159_OBUSIZE_ANNEXB
      *num_streams = read_multi_stream_HEADER(&rb, stream_ids);
    } else {
#if CONFIG_F159_OBUSIZE_ANNEXB
      std::vector<uint8_t> obu_size_data(length_field_size);
      size_t coded_obu_size;
      ...
      write_obu_header_with_stream_id(obu_header_data.data(), &obu_header, 0);
...
int main(int argc, const char *argv[]) {
  ...
  int num_streams = 1;
  int stream_ids[MAX_NUM_STREAMS] = { -1, -1, -1, -1 };
  ...
  char *output_file_names[MAX_NUM_STREAMS];
  ...
  FILE *fout[MAX_NUM_STREAMS];

  generate_filenames(base_filename, MAX_NUM_STREAMS, output_file_names);
  ...
  for (int i = 0; i < MAX_NUM_STREAMS; ++i) unit_number[i] = 0;
